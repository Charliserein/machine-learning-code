...
#1、多项朴素贝叶斯方法
model = MultinomialNB(alpha=100e-2)#采取多项朴素贝叶斯方法，不进行拉普拉斯修正
model.fit(X_train, y_train)#基于训练样本，使用fit方进行拟合
model.score(X_test, y_test)#基于测试样本，计算模型预测准确率

model = MultinomialNB(alpha=100e-2)#采取多项朴素贝叶斯方法，进行拉普拉斯修正
model.fit(X_train, y_train)#基于训练样本，使用fit方进行拟合
model.score(X_test, y_test)#基于测试样本，计算模型预测准确率
#2、补集朴素贝叶斯方法
model = ComplementNB(alpha=100e-2)#采取补集朴素贝叶斯方法，进行拉普拉斯修正
model.fit(X_train, y_train)#基于训练样本，使用fit方进行拟合
model.score(X_test, y_test)#基于测试样本，计算模型预测准确率
#3、二项朴素贝叶斯方法
model = BernoulliNB(alpha=100e-2)#采取二项朴素贝叶斯方法，进行拉普拉斯修正
model.fit(X_train, y_train)#基于训练样本，使用fit方进行拟合
model.score(X_test, y_test)#基于测试样本，计算模型预测准确率

model = BernoulliNB(binarize=2, alpha=100e-2)#采取二项朴素贝叶斯方法，设置参数binarize=2，进行拉普拉斯修正
model.fit(X_train, y_train)#基于训练样本，使用fit方进行拟合
model.score(X_test, y_test)#基于测试样本，计算模型预测准确率
