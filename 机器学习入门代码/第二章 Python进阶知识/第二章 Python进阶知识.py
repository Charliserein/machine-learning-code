...
#2.1.2 循环语句
#遍历字符串
strname='对酒当歌,人生几何'#创建字符串strname
print(strname)#显示字符串strname的内容
for ch in strname:#通过for循环遍历字符串strname中的字符
    print(ch)#输出字符
#进行数值循环计算
for x in range(1,5,1):#针对range(1,5,1)中的值开展以下for循环计算
    x=x*2#将range(1,5,1)中的原值乘以2得到新值
    print(x,end='/')#输出新值，其中参数“,end='分隔符'”表示将结果显示为1行，本例中分隔符为“/”

#2.2 python函数（function）
#2.2.1 函数的创建和调用
def area(width, length):#定义长方形面积函数area，参数为宽width和长height
    return width*length#返回宽width乘以长height的结果
area(6,6)#调用长方形面积函数area，参数为4和6
#2.2.2 参数相关概念与操作
def param_test(obj):
    obj += obj
    print('形参值为:', obj)
...
def aweights(width, length):#定义width和length的加权值
    return 0.3*width+0.7*length#返回0.3*width+0.7*length的结果
aweights(length=10,width=6)#指定关键字参数

#定义printcompany（）函数，作用为输出上市公司名称
def printcompany(*company):
    print('\n2022年新能源上市公司：')
    for item in company:
        print(item)
printcompany('宁德时代 300750') #指定实际参数       
printcompany('宁德时代 300750','比亚迪 002594','国轩高科 002074') #重新指定实际参数       
printcompany('宁德时代 300750','国轩高科 002074')   #再次重新指定实际参数       
    
list1=['宁德时代 300750','比亚迪 002594','国轩高科 002074']#创建列表list1
printcompany(*list1)#将列表list1作为参数传递给printcompany（）函数

#定义printcompany（）函数，作用为输出上市公司名称
def printcompany(**company):#定义printcompany（）函数
    print('\n2022年新能源上市公司：')#打印'\n2022年新能源上市公司：'
    for key,value in company.items():#遍历字典company
        print("[" + key + "] 的股票代码是："+value)#输出函数运行结果
printcompany(宁德时代='300750') #指定实际参数       
printcompany(宁德时代='300750',比亚迪='002594',国轩高科='002074') #重新指定实际参数       
   
    
dict1={'宁德时代':'300750','比亚迪':'002594','国轩高科':'002074'}#创建元组dict1
printcompany(**dict1)#将元组dict1作为参数传递给printcompany（）函数

#2.2.3 变量的作用域
def area(width, length):#定义长方形面积函数area，参数为宽width和长height
    areameasure=width*length#生成变量areameasure，为宽width乘以长height
    print(areameasure)#输出areameasure的结果
#print(areameasure)#输出areameasure的结果

poetry='对酒当歌，人生几何'#生成全局变量poetry
def poetryprint():#定义函数poetryprint()
    print(poetry)#输出poetry的结果
poetryprint()
print(poetry)#输出poetry的结果


def poetryprint():#定义函数poetryprint()
    global poetry#将poetry声明为全局变量
    poetry='对酒当歌，人生几何'#生成全局变量poetry
    print(poetry)#输出poetry的结果
poetryprint()
print(poetry)#输出poetry的结果

#2.3 python模块（module）和包（Package）
#2.3.1 模块的创建和导入
#一、使用import语句导入模块
import pandas as pd #导入模块pandas，并将其简称为pd
#import pandas,numpy#一次性导入模块pandas和numpy
data=pd.read_csv('C:/Users/Administrator/.spyder-py3/数据4.1.csv')#读取数据4.1.csv，并命令为data
import sys#调用模块sys
print(sys.path)#输出sys.path
#二、使用from……import语句导入模块
#from sklearn.neighbors import KNeighborsClassifier, RadiusNeighborsClassifier
#from sklearn.neighbors import *#导入模块sklearn.neighbors中的全部内容
print(dir())#查看模块sklearn.neighbors中的全部内容
#三、用户自行创建模块并导入
...
data=pd.read_csv('C:/Users/Administrator/.spyder-py3/数据4.1.csv')#读取数据4.1.csv，并命令为data
import sys#调用模块sys
sys.path.append(r"C:\Users\Administrator\.spyder-py3")#设置sys路径
import normcheck#从sys路径中导入normcheck模块
normcheck.normality_check(data)#使用导入的normcheck模块分析data

#2.4 python numpy模块数组（array）
#数组的创建
...
#2.6 python对象（object）与类（class）
#定义类
class company:#定义类的名称为company
    def __init__(self, name, credit_rating='BBB', loan=5000):#使用__init__函数，只要调用此类，就会自动运行该函数；self即为调用函数对象本身,类中有三个参数，name、credit_rating、loan，后两个参数分别设置了默认值
        self.name = name#定义对象self的属性name
        self.credit_rating = credit_rating#定义对象self的属性credit_rating
        self.loan=loan#定义对象self的属性loan
SDLN=company('山东 XX 有限责任公司')#将'山东 XX 有限责任公司'传参给类company，使其实例化
SDLN.name#查看SDLN的参数name的值
SDLN.credit_rating#查看SDLN的参数credit_rating的值
SDLN.loan#查看SDLN的参数loan的值

HBDB= company('河北 XX 有限责任公司', credit_rating='AA', loan=15000)#将'河北 XX 有限责任公司'传参给类company，使其实例化，但对参数credit_rating、loan进行了赋值
HBDB.name#查看HBDB的参数name的值
HBDB.credit_rating#查看HBDB的参数credit_rating的值
HBDB.loan#查看HBDB的参数loan的值
#定义适用于类对象的方法
class company:#定义类的名称为company
    def __init__(self, name, credit_rating='BBB', loan=5000):#使用__init__函数，只要调用此类，就会自动运行该函数；self即为调用函数对象本身,类中有三个参数，name、credit_rating、loan，后两个参数分别设置了默认值
        self.name = name#定义对象self的属性name
        self.credit_rating = credit_rating#定义对象self的属性credit_rating
        self.loan=loan#定义对象self的属性loan
    def province(self):#定义函数province（）
        return self.name.split()[0]#返回self的属性name按分隔符分隔的第一部分
    def company_Nature(self):#定义函数company_Nature（）
        return self.name.split()[-1]#返回self的属性name按分隔符分隔的最后一部分
    def quantity(self,percent):#定义函数quantity（）
        self.loan = round(self.loan * (1 + percent),1)#返回self的属性loan，根据参数percent，将初始loan提高一定百分比
    def __repr__(self):#定义函数__repr__（）
        return f'company: {self.name}. credit_rating: {self.credit_rating}. loan: {self.loan}.'#以f'字符串形式定制print（）函数输出结果，包括name、credit_rating、loan信息。
        
HBDB= company('河北 XX 有限责任公司', credit_rating='AA', loan=15000)#将'河北 XX 有限责任公司'传参给类company，使其实例化，并对参数credit_rating、loan进行了赋值
HBDB.province()#查看HBDB的province的值
HBDB.company_Nature()#查看HBDB的company_Nature的值
HBDB.quantity(0.3)#设定percent为0.3，查看HBDB的quantity的值
HBDB.loan#查看HBDB的参数loan的值
print(HBDB)#查看HBDB的定制输出信息

#将类对象使用到子类
class credit_Line(company):#定义类的名称为credit_Line(company)，该类为company的子类
    def quantity(self, percent, increase=0.1):#定义函数quantity（），参数除了percent之外，还有一个固定参数increase=0.1
        company.quantity(self, percent + increase)#将初始loan按照percent和increase（固定为0.1）之和提高一定百分比

ZJSW=credit_Line('河北 XX 股份有限公司', 'AAA', 30000)#将'河北 XX 有限责任公司'传参给类company，使其实例化，并对参数credit_rating、loan进行了赋值
ZJSW.name#查看ZJSW的参数name的值
ZJSW.credit_rating#查看ZJSW的参数credit_rating的值
ZJSW.loan#查看ZJSW的参数loan的值
ZJSW.province()#查看ZJSW的province的值
ZJSW.company_Nature()#查看ZJSW的company_Nature的值
ZJSW.quantity(0.4)#设定percent为0.4，查看ZJSW的quantity的值
ZJSW.loan#查看ZJSW的参数loan的值

#2.7 python数据读取
#2.7.1读取csv或者txt
import pandas as pd#导入pandas模块，并简称为pd
data=pd.read_csv('C:/Users/Administrator/.spyder-py3/数据4.1.csv')
data=pd.read_csv('C:/Users/Administrator/.spyder-py3/数据4.1.txt')
data=pd.read_csv('C:/Users/Administrator/.spyder-py3/数据4.1.txt',sep='\t')#从设置路径中读取数据4.1，数据4.1为.csv格式
data=pd.read_csv('C:/Users/Administrator/.spyder-py3/数据4.1.txt',sep='\s+')#从设置路径中读取数据4.1，数据4.1为.csv格式
data=pd.read_table('C:/Users/Administrator/.spyder-py3/数据4.1.txt')
data=pd.read_csv('C:/Users/Administrator/.spyder-py3/数据4.1.csv', header=None)
data=pd.read_csv('C:/Users/Administrator/.spyder-py3/数据4.1.csv',names = ['V1', 'V2', 'V3', 'V4'])
data=pd.read_csv('C:/Users/Administrator/.spyder-py3/数据4.1.csv', skiprows=[0],names = ['V1', 'V2', 'V3', 'V4'])
...
data=pd.read_excel('C:/Users/Administrator/.spyder-py3/数据4.1.xlsx')[['profit','invest']] # 读取并筛选几列

#2.7.3读取spss数据
#pip install --upgrade pyreadstat#安装pyreadstat，大家在运行时把最前面的#号去掉
import pandas as pd#导入pandas模块，并简称为pd
data=pd.read_spss('C:/Users/Administrator/.spyder-py3/数据7.1.sav')

#2.7.4读取stata数据
import pandas as pd#导入pandas模块，并简称为pd
data=pd.read_stata('C:/Users/Administrator/.spyder-py3/数据8.dta')
    
#2.8 python数据检索
data=pd.read_csv('C:/Users/Administrator/.spyder-py3/数据2.1.csv')#读取数据2.1
data.describe()   # 描述统计
data.info()    # 基本信息
data.dtypes    #　列格式类型
data.head(2)    #前n行
...
#二、用前后值填充缺失数据
data=pd.read_csv('C:/Users/Administrator/.spyder-py3/数据2.1.csv')#重新读取数据2.1
...
#2.10 python数据重复值处理
#2.10.1查看数据集中的重复值
data=pd.read_csv('C:/Users/Administrator/.spyder-py3/数据2.2.csv')#读取数据2.2
